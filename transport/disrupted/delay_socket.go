...
type DelayFunction func(t time.Time) time.Duration
	...
	listener := func() {
		...
		for {
			...
			default:
				...
				if err == nil {
					time.Sleep(s.delayFunc(time.Now()))
					s.delayedPackets <- p
				}
			}
		}
...
func FixedDelay(value time.Duration) DelayFunction {
	return func(t time.Time) time.Duration { return value }
}
...
func ExponentialDelay(mean time.Duration) DelayFunction {
	return func(t time.Time) time.Duration {
		return time.Duration(int(randGen.ExpFloat64()*float64(mean.Microseconds()))) * time.Microsecond
	}
}
